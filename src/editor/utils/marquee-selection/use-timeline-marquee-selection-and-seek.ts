...
export const useTimelineMarqueeSelectionAndSeek = ({
	...
	const onPointerDown: React.PointerEventHandler<HTMLDivElement> = useCallback(
		(initialPointerEvent) => {
			...
			// DOM 기반으로 Seek 영역인지 확인 (Ticks 영역 클릭 시)
			const target = initialPointerEvent.target as HTMLElement;
			const isSeekArea = target.closest('[data-timeline-ticks]') !== null ||
				target.closest('[data-timeline-seek-area]') !== null;
			
			if (isSeekArea) {
				startTimelineSeek(initialPointerEvent as unknown as React.MouseEvent<HTMLDivElement>);
				return;
			}
