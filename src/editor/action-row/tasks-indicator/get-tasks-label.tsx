...
export const getTasksLabel = (
	...
	if (assetDownloadTasks.length > 0) {
		...
		const bytesRemaining = Number.isFinite(task.bytesRemaining) ? task.bytesRemaining : 0;
		const totalSize = Number.isFinite(task.totalSize) && task.totalSize > 0 ? task.totalSize : 1;
		return {
			label: `Downloading ${formatBytes(bytesRemaining)}`,
			progress: {
				type: 'numeric-progress',
				progress: 1 - bytesRemaining / totalSize,
			},
		};
	}
