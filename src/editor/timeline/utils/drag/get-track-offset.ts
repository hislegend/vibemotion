...
export const getTrackOffset = ({
	...
	for (
		...
		// 경계 보호: create-at-top/bottom이 우선되어야 하는 가장자리에서는 insert-between을 허용하지 않음
	...
	// 우선순위 2: edge track에 도달했고 남은 거리가 있는지 확인
	if (direction === 'up') {
	...
	// 우선순위 3: 기본 케이스 - 현재 track 범위 내에서 기존 track으로 이동
	return {type: 'move', direction, trackOffset};
};
